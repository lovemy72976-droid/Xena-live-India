...
function getJwtSecret(){
  ...
  try {
    ...
    return generated;
  } catch { return crypto.randomBytes(48).toString('base64url'); }
}
const JWT_SECRET = getJwtSecret();
const DB_FILE = process.env.DATA_FILE || path.join(DATA_DIR, 'data.json');
const OAUTH_STATE_TTL_MS = 10 * 60 * 1000;
const oauthStates = new Map();
function oauthState(provider){ const state=crypto.randomBytes(24).toString('hex'); oauthStates.set(state,{provider,expiresAt:Date.now()+OAUTH_STATE_TTL_MS}); return state; }
function takeOAuthState(state,provider){ const x=oauthStates.get(state); oauthStates.delete(state); return x && x.provider===provider && x.expiresAt>Date.now(); }
function oauthUser({provider,providerId,email,name}){
  let u=db.users.find(x=>x.oauthProvider===provider && x.oauthId===providerId);
  if(!u && email) u=db.users.find(x=>x.email && x.email.toLowerCase()===String(email).toLowerCase());
  if(!u){
    let base=String(name||provider+'user').replace(/[^a-zA-Z0-9_]/g,'').slice(0,18)||provider+'user';
    let username=base, n=1; while(db.users.some(x=>x.username.toLowerCase()===username.toLowerCase())) username=base+(n++);
    u={id:id('u'),mobile:null,username,email:email||null,passwordHash:null,oauthProvider:provider,oauthId:providerId,coins:12050,diamonds:560,followers:0,following:0,likes:0,createdAt:Date.now()};
    db.users.push(u);
  } else { u.oauthProvider=provider; u.oauthId=providerId; if(email)u.email=email; if(name && u.username===(''+provider+'user'))u.username=name; }
  saveDb(); return u;
}
function oauthRedirect(token){ return 'xena://oauth?token='+encodeURIComponent(token); }


function loadDb(){
  if(!fs.existsSync(DB_FILE)) return {users:[], rooms:[], messages:[], transactions:[], follows:[], blocks:[], reports:[]};
  try { return JSON.parse(fs.readFileSync(DB_FILE,'utf8')); } catch { return {users:[],rooms:[],messages:[],transactions:[],follows:[],blocks:[],reports:[]}; }
}
let db = loadDb();
db.blocks=db.blocks||[]; db.reports=db.reports||[];
...
// Same-origin is the secure default. Enable explicit CORS only when needed by a separate web client.
...
app.get('/auth/google',(req,res)=>{
  ...
  if(!clientId||!secret||!redirect) return res.status(503).send('Google Login is not configured on this server.');
  const state=oauthState('google');
  const q=new URLSearchParams({client_id:clientId,redirect_uri:redirect,response_type:'code',scope:'openid email profile',access_type:'offline',prompt:'select_account',state});
  res.redirect('https://accounts.google.com/o/oauth2/v2/auth?'+q.toString());
});
app.get('/auth/google/callback',async(req,res)=>{
  try{
    if(!takeOAuthState(req.query.state,'google')) return res.status(400).send('Invalid or expired OAuth state.');
    if(req.query.error) return res.status(400).send('Google login cancelled.');
    ...
    const {tokens}=await oauth2.getToken(String(req.query.code)); oauth2.setCredentials(tokens);
    const ticket=await oauth2.getTokenInfo(tokens.access_token); const info=await fetch('https://openidconnect.googleapis.com/v1/userinfo',{headers:{Authorization:'Bearer '+tokens.access_token}}).then(r=>r.json());
    const u=oauthUser({provider:'google',providerId:String(info.sub||ticket.sub),email:info.email,name:info.name||info.given_name});
    res.redirect(oauthRedirect(tokenFor(u)));
  }catch(e){console.error('google oauth',e);res.status(502).send('Google login failed.');}
});
app.get('/auth/facebook',(req,res)=>{
  const appId=process.env.FACEBOOK_APP_ID, secret=process.env.FACEBOOK_APP_SECRET, redirect=process.env.FACEBOOK_REDIRECT_URI;
  if(!appId||!secret||!redirect) return res.status(503).send('Facebook Login is not configured on this server.');
  const state=oauthState('facebook'); const q=new URLSearchParams({client_id:appId,redirect_uri:redirect,state,scope:'email,public_profile'});
  res.redirect('https://www.facebook.com/v23.0/dialog/oauth?'+q.toString());
});
app.get('/auth/facebook/callback',async(req,res)=>{
  try{
    if(!takeOAuthState(req.query.state,'facebook')) return res.status(400).send('Invalid or expired OAuth state.');
    if(req.query.error) return res.status(400).send('Facebook login cancelled.');
    const base='https://graph.facebook.com/v23.0/oauth/access_token';
    const q=new URLSearchParams({client_id:process.env.FACEBOOK_APP_ID,client_secret:process.env.FACEBOOK_APP_SECRET,redirect_uri:process.env.FACEBOOK_REDIRECT_URI,code:String(req.query.code)});
    const tok=await fetch(base+'?'+q.toString()).then(r=>r.json()); if(!tok.access_token) throw new Error('No Facebook access token');
    const info=await fetch('https://graph.facebook.com/me?fields=id,name,email&access_token='+encodeURIComponent(tok.access_token)).then(r=>r.json()); if(!info.id)throw new Error('Facebook profile unavailable');
    const u=oauthUser({provider:'facebook',providerId:String(info.id),email:info.email,name:info.name}); res.redirect(oauthRedirect(tokenFor(u)));
  }catch(e){console.error('facebook oauth',e);res.status(502).send('Facebook login failed.');}
});

app.get('/health',(req,res)=>res.json({ok:true,name:'Xena Live India',version:'3.1.0',storage:process.env.DATA_FILE?'file':'managed-data-dir',time:new Date().toISOString()}));
app.get('/api/version',(req,res)=>res.json({name:'Xena Live India',version:'3.1.0'}));
app.get('/api/config',(req,res)=>res.json({iceServers:[{urls:['stun:stun.l.google.com:19302']},...(process.env.TURN_URL?[{urls:process.env.TURN_URL.split(',').map(x=>x.trim()),username:process.env.TURN_USERNAME||'',credential:process.env.TURN_CREDENTIAL||''}]:[])]}));
...
app.post('/api/users/:userId/block',auth,(req,res)=>{
  const me=findUser(req.user.id), target=findUser(req.params.userId);
  if(!target) return res.status(404).json({error:'User not found'});
  if(me.id===target.id) return res.status(400).json({error:'Cannot block yourself'});
  const exists=db.blocks?.some(b=>b.from===me.id&&b.to===target.id);
  db.blocks=db.blocks||[];
  if(exists) db.blocks=db.blocks.filter(b=>!(b.from===me.id&&b.to===target.id));
  else db.blocks.push({from:me.id,to:target.id,createdAt:Date.now()});
  saveDb(); res.json({blocked:!exists});
});

app.post('/api/reports',auth,(req,res)=>{
  const {targetUserId,roomId,messageId,reason}=req.body||{};
  if(!reason || String(reason).trim().length<3) return res.status(400).json({error:'Report reason is required'});
  db.reports=db.reports||[];
  const report={id:id('report'),reporterId:req.user.id,targetUserId:targetUserId||null,roomId:roomId||null,messageId:messageId||null,reason:String(reason).slice(0,500),status:'open',createdAt:Date.now()};
  db.reports.push(report); saveDb(); res.json({ok:true,reportId:report.id});
});

app.delete('/api/me',auth,(req,res)=>{
  const uid=req.user.id;
  db.users=db.users.filter(u=>u.id!==uid);
  db.rooms=db.rooms.filter(r=>r.hostId!==uid);
  db.messages=db.messages.filter(m=>m.userId!==uid);
  db.transactions=db.transactions.filter(t=>t.userId!==uid && t.senderId!==uid && t.receiverId!==uid);
  db.follows=db.follows.filter(f=>f.from!==uid&&f.to!==uid);
  db.blocks=(db.blocks||[]).filter(b=>b.from!==uid&&b.to!==uid);
...
app.post('/api/billing/verify',auth,async(req,res)=>{
  try{
    ...
    const coinMap={coins_100:100,coins_500:500,coins_1000:1000,coins_5000:5000};
    const amount=coinMap[productId];
    if(!amount||!purchaseToken)return res.status(400).json({error:'Invalid purchase'});
    if(db.transactions.some(t=>t.type==='play_purchase'&&t.purchaseToken===purchaseToken)) return res.json({ok:true,alreadyProcessed:true});
    const raw=process.env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON;
    if(!raw)return res.status(503).json({error:'Google Play verification is not configured'});
    ...
    const packageName=process.env.ANDROID_PACKAGE_NAME||'com.xenalive.india';
    const result=await publisher.purchases.products.get({packageName,productId,token:purchaseToken});
    const purchase=result.data;
    if(Number(purchase.purchaseState)!==0)return res.status(400).json({error:'Purchase is not completed'});
    const u=findUser(req.user.id); if(!u)return res.status(404).json({error:'User not found'});
    u.coins+=amount; db.transactions.push({id:id('tx'),userId:u.id,type:'play_purchase',amount,productId,purchaseToken,createdAt:Date.now(),description:`Google Play ${productId}`}); saveDb();
    try{await publisher.purchases.products.consume({packageName,productId,token:purchaseToken});}catch(e){}
    res.json({ok:true,coins:u.coins,credited:amount});
  }catch(e){console.error('billing verify',e);res.status(502).json({error:'Purchase verification failed'});}
});
...
const ioOptions=corsOrigins.length?{cors:{origin:corsOrigins,credentials:true}}:{};
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Xena Live server listening on ${PORT}`));
